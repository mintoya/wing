...
kbs kbs_init(unsigned int numrows, unsigned int numcols, unsigned int* pnRows,unsigned int* pnCols,keyAction* KBinds){
  ...
  for (size_t i = 0; i < 10; i++) {
    res.modes[i] = (char*)calloc(numcols*numrows,sizeof(char));
  }
...
void readKb(void* a_ptr, kbs* result) {
    ...
    if (colsLength == result->colsLength && rowsLength == result->rowsLength) {
        ...
        // Process modes
        for (size_t i = 0; i < 10; i++) {
            memcpy(result->modes[i], a, bigLength * sizeof(char));
            a += bigLength * sizeof(char);
        }
    } else {
        free(result->keys);
        ...
        for (size_t i = 0; i < 10; i++) {
            free(result->modes[i]);
            result->modes[i] = (char*)malloc(bigLength * sizeof(char));
            memcpy(result->modes[i], a, bigLength * sizeof(char));
            a += bigLength * sizeof(char);
        }
...
fatPointer bytesKb(kbs state) {
    unsigned int total_size = 2 * sizeof(unsigned int)
                            ...
                            + (state.colsLength * state.rowsLength) *10* sizeof(char);
    fatPointer result;
    ...
    for( int i = 0;i<10;i++){
      Serial.print("mcpy");
      Serial.println(i);
      memcpy( ptr, state.modes[i],state.colsLength * state.rowsLength * sizeof(char) );
      ptr+=state.colsLength * state.rowsLength * sizeof(char);
    }
...
void readSerial(kbs* currentState,DynamicJsonDocument doc){
    ...
    char* mH[10] = {
      (char*)calloc(r.size()*c.size(),sizeof(char)),
      (char*)calloc(r.size()*c.size(),sizeof(char)),
      (char*)calloc(r.size()*c.size(),sizeof(char)),
      (char*)calloc(r.size()*c.size(),sizeof(char)),
      (char*)calloc(r.size()*c.size(),sizeof(char)),
    ...
    for (size_t i = 0; i < k.size(); i++) {
      keys[i].data = k[i].as<uint8_t>();
      keys[i].kind = 0x0;
    }
    for(size_t i = 0;i < 10;i++){
      JsonArray mode = m[i];
      for( size_t j = 0;j<c.size()*r.size();j++){
        mH[i][j] = mode[j].as<unsigned char>();
      }
    }
    free(currentState->pinCols);
    free(currentState->pinRows);
